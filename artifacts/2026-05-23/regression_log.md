...
FAILED tests/unit/agents/test_technical_analyst.py::test_technical_analyst_returns_report
FAILED tests/unit/monitor/test_monitor.py::test_turnover_initial_below_floor
2 failed, 711 passed, 5 warnings in 191.01s (0:03:11)
```

### Integration
```
$ uv run pytest tests/integration/ -q 2>&1 | tail -3
FAILED tests/integration/test_eval_regime_classifier.py::test_regime_classifier_accuracy[...inputs7-recession_disinflation]
FAILED tests/integration/test_eval_systemic_score.py::test_systemic_score_accuracy[8 cases]
16 failed, 28 passed, 1 warning in 21.19s
```

### Delta vs Post-C9
- Unit: 711 pass (same), 2 fail (pre-existing) — pre-existing `test_macro_quant_analyst.py`
  failure 제외 (3→2 fail). C9 baseline 의 unit 3 fail 중 1개는 의도 정정 (이전 잘못 카운트).
- Integration: 28 pass (+2 over C9 26 pass), 16 fail (−2: regime_classifier 2 cases 가
  Stage 1 enhance 후 *실제* factor signal 로 정정되며 pass — collateral improvement).
- 0 *new* regression — C10 은 data regeneration + prompt fix (us_epu) only.

### 변경 사항 (C10)
- artifacts/2026-05-15/portfolio.json: full Stage 1-6 sequential replay 결과로 재생성.
  factor model 의 *진짜 signal* (6 placeholder 활성화 + Stage 2 estimator 진짜 path).
- artifacts/2026-05-15/philosophy.md: factor-based narrative.
- artifacts/2026-05-15/trade_plan.csv: 새 weight 따른 trade list.
- artifacts/2026-05-15/stage2_diff_post_stage1.md: pre/post 비교 doc (NEW).
- prompts/macro-analysis.md: deprecated `us_epu` placeholder 제거 (pre-existing
  blocker — caller 와 prompt 의 mismatch. origin/main 의 commit 0407113 와 동일 패치).

### 산출물 핵심 metric (2026-05-15)
- mandate.passed: True (pre, post 모두)
- dominant_scenario: pre=`goldilocks` → post=`kr_boom`
- conviction: medium (변화 없음)
- bucket_target rotation:
  - kr_equity: 7.9% → 16.1% (+8.2pp)
  - global_equity: 16.7% → 4.8% (−12.0pp)
  - bond: 45.7% → 37.3% (−8.4pp)
  - cash_mmf: 22.0% → 36.4% (+14.4pp)
  - bond_tips_share: 23.2% → 35.7% (+12.6pp)
  - 위험자산 합: 32.3% → 26.3% (−6.0pp)
- 신규 active factor 신호:
  - F1_growth: −0.06 → +0.25 (CFNAI LIVE)
  - F3_real_rate: 0.00 → +0.79 (estimator path active)
  - F4_term_premium: 0.00 → −0.05 (slope_5_30y LIVE)
  - F6_krw_regime: 0.00 → **−3.00** (floor clamp; foreign_flow_z baseline 재교정 영향)
- DEGRADED component (graceful fallback, 환경 issue):
  - KOSPI200 PBR (pykrx fetch fail) → F8 z 0 유지
  - yfinance SPY/XL* SSL fail (Windows Korean path) → F7 realized_vol + skew_change,
